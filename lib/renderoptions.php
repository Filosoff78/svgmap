...
class RenderOptions
{
    ...
    function renderInput($Option, $arControllerOption, $fieldName, $val)
    {
        ...
        else if($type[0]=="tables") {
        $tables = \PGK\Maps\MapsTable::query()
            ->setSelect(['NAME', 'MAP', 'ID'])
            ->fetchAll();
        ?>
        <?foreach ($tables as $table):?>
            <?=$table['ID']?>
            <?=$table['NAME']?>
            <a href="<?=CFile::GetPath($table['MAP'])?>" target="_blank"><?=$table['MAP']?></a>
            <br>
        <?endforeach;?>
        <?
        }
